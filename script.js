...
(() => {
  const el = $('#typed-role');
  if (!el) return;
  const words = ['API system', 'basic python', 'a database', 'How to clear bug'];
...
if (backToTop) {
  window.addEventListener('scroll', () => {
    backToTop.style.display = window.scrollY > 320 ? 'flex' : 'none';
  });
...
function randomJump() {
  ...
  const jumpDuration = 2000 + Math.random() * 1000;
  ...
  const nextJump = 3000 + Math.random() * 2000;
  setTimeout(randomJump, nextJump);
}

window.addEventListener('load', () => setTimeout(randomJump, 1500));
...
if (chatForm) {
  chatForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      botMsg.textContent = data.reply || "RAI lagi mikir dulu 😅";
      chatBox.appendChild(botMsg);
      chatBox.scrollTop = chatBox.scrollHeight;
    } catch (err) {
      console.error(err);
      const botMsg = document.createElement("div");
      botMsg.className = "chat bot error";
      botMsg.textContent = "⚠️ RAI gagal jawab, coba lagi ya~";
      chatBox.appendChild(botMsg);
    }
  });
}
...
closeBtn.addEventListener("click", () => chatBoxContainer.style.display = "none");
...
window.addEventListener('scroll', () => {
  if (window.scrollY > lastScrollY) navWrap.classList.add('hide');
...
document.addEventListener('mousemove', (e) => {
  if (e.clientY <= 60) navWrap.classList.remove('hide');
});